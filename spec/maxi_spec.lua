...
local to_f, to_str = maxi(require "modal", true)
...
describe("subcycle", function()
   it("should parse mini subcycle as a first class", function()
      assert.pat(fastcat { "bd" }, eval "[bd]")
      assert.pat(fastcat { "bd", "sd" }, eval "[bd sd]")
   end)
end)

describe("stack", function()
   it("should parse mini stack as a first class", function()
      assert.pat(stack { pure "bd", pure "sd" }, eval "[bd, sd]")
      assert.pat(stack { fastcat { "bd", "bd" }, fastcat { "sd", "sd" } }, eval "[bd bd, sd sd]")
   end)
end)

describe("slow_seq", function()
   it("should parse mini slow_seq as a first class", function()
      assert.pat(slowcat { "bd" }, eval "<bd>")
      -- assert.pat(slowcat { "bd", "sd" }, eval "<bd sd>")
      -- assert.pat(slowcat { 0, 1, 2, 3, 4 }, eval "<0 .. 4>")
   end)
end)

describe("polymeter", function()
   it("should parse mini polymeter as a first class", function()
      assert.pat(fastcat { "bd", "sd", "hh" }, eval "{bd sd hh}")
      assert.pat(fastcat { "bd", "sd", "hh" }, eval "{bd sd hh}%3")
      assert.pat(fastcat { "bd", "sd" }, eval "{bd sd hh}%2")
      assert.pat(stack { fastcat { "bd", "sd" }, fastcat { 1, 2 } }, eval "{bd sd hh, 1 2 3 4}%2")
   end)
end)
...
describe("dotStack", function()
   it("should parse mini dotStack as first class", function()
      assert.pat(fastcat { fastcat { 1, 2 }, fastcat { 3, 4 } }, eval "[1 2 . 3 4]")
   end)
   it("should parse mini dotStack as first class", function()
      assert.pat(slowcat { fastcat { 1, 2 }, fastcat { 3, 4 } }, eval "<1 2 . 3 4>")
   end)
end)

describe("ops", function()
   it("should parse mini ops as a first class", function()
      assert.pat(fast(2, "bd"), eval "bd*2")
      assert.pat(euclidRot(3, 8, 0, "bd"), eval "bd(3,8)")
      assert.pat(degradeBy(0.5, "bd"), eval("bd?", "degradeBy"))
      assert.pat(fastcat { "bd", "bd", "sd" }, eval "[bd! sd]")
      assert.pat(pure { 0.3, 0.5, 2 }, eval "0.3:0.5:2")
   end)
   it("weight", function()
      assert.pat(timecat { 2, "bd", 1, "sd" }, eval "[bd@2 sd]")
      assert.pat(timecat { 3, "bd", 2, "sd" }, eval "[bd __ sd _]")
      assert.pat(arrange { 2, "bd", 1, "sd" }, eval "<bd@2 sd>")
   end)
end)

describe("tidal ops", function()
   it("should parse tidal ops as a first class", function()
      assert.pat(note(3), eval_top [[note 2 +| note 1]])
      assert.pat(pure { s = "bd", room = "0.2" }, eval_top [[s bd |> room 0.2]])
      assert.pat(pure { s = "bd", room = "0.2" }, eval_top [[s bd >|| room 0.2]])
   end)
end)
...
describe("list(p)", function()
   ...
   it("should parse sexp func call as a first class", function()
      assert.pat(fast(1, 2), eval "(fast 1 2)")
      assert.pat(fast(1, pure(2)), eval "(fast 1 (pure 2))")
   end)
   it("should parse nested function calls", function()
      assert.same(fast(2, 1)(1, 2), eval "((fast 2 1) 1 2)")
   end)
