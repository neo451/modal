...
describe("new", function()
   it("should create with defaults", function()
      local t = Span()
      assert.are.same(Time(1), t.start)
      assert.are.same(Time(1), t.stop)
   end)
   it("should create with args", function()
      local t = Span(3, 4)
      assert.are.same(Time(3), t.start)
      assert.are.same(Time(4), t.stop)
   end)
   it("should promote numbers to fractions", function()
      local t = Span(0.5, 0.75)
      assert.are.same(Time(1, 2), t.start)
      assert.are.same(Time(3, 4), t.stop)
   end)
end)
describe("spanCycles", function()
   it("should break multi cycle span into pieces", function()
      ...
      assert.are.same(Time(3, 4), spans[1].start)
      assert.are.same(Time(1, 1), spans[1].stop)
      assert.are.same(Time(1, 1), spans[2].start)
      assert.are.same(Time(2, 1), spans[2].stop)
      assert.are.same(Time(2, 1), spans[3].start)
      assert.are.same(Time(3, 1), spans[3].stop)
      assert.are.same(Time(3, 1), spans[4].start)
      assert.are.same(Time(7, 2), spans[4].stop)
   end)
   it("should preserve subcycle length spans", function()
      local t = Span(1 / 16, 1)
      local spans = t:spanCycles()
      assert.are.same(1, #spans)
      assert.are.same(Time(1, 16), spans[1].start)
      assert.are.same(Time(1, 1), spans[1].stop)
   end)
end)
